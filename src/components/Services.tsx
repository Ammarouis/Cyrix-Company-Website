...
const Services = () => {
  ...
  const renderCard = (service: ReturnType<typeof getFeaturedServices>[number]) => (
    <Link to={`/service/${service.id}`} className="block">
      ...
      <div className="p-6">
        <h3 className="text-base font-normal mb-1 text-card-foreground tracking-tight">{service.name}</h3>
        <div className="flex items-center gap-1 text-muted-foreground mb-4 text-xs font-light">
          <span>{service.tagline}</span>
        </div>
        <div className="flex flex-wrap gap-1.5 mb-5">
          {service.features.map((feature) => (
            <span key={feature} className="text-[10px] uppercase tracking-wide px-2 py-1 bg-accent/15 text-primary rounded-sm font-light">
              {feature}
            </span>
          ))}
        </div>
        <div className="flex items-center justify-between">
          <span className="text-sm font-light text-muted-foreground">{service.location.split(" • ").length} {t("services.coreCapabilities")}</span>
  ...
  return (
    <section id="services" className="py-32 lg:py-40 bg-background" ref={ref}>
